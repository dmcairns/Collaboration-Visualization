...
vertices1 <- vertices1 %>%
  mutate(newValue=case_when(
    (from=="BG;IG") & (shortName=="BG;IG;TF") ~ 3,
    (from=="BG;TF") & (shortName=="BG;TF;ZZ") ~ 3,
    (from=="CB;TF") & (shortName=="CB;TF;WJ") ~ 3,
    (from=="CL;DC") & (shortName=="CL;DC") ~ 7,
    (from=="BR;GA;OF;SQ") & (shortName=="BR;GA;OF;SQ") ~ 7,
    (from=="CL;DC;JJ;KG") & (shortName=="CL;DC;JJ;KG") ~ 7,
    TRUE ~ newValue
  ))
...
outGraph <- ggraph(mygraph, layout = 'circlepack', weight=size) +
  ...
  scale_color_manual( values=c("top" = "green", "1" = "blue", "2" = "yellow", "3" = "white", "4"="black") ) +
  theme_void() +
